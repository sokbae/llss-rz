...
if (graph==1){
    ...
    pdf(file=paste("../output/graph-thr-het-",n,".pdf",sep=''))
    plot(qs,lr,lty=1,col=1,xlim=xxlim,ylim=yylim,type="l",ann=0)
    lines(qs,clr,lty=2,col=2)     
    title(main=rbind("sup-Wald Test For Threshold",
               "Reject Linearity if the Sequence Exceeds Critical Value"),
          xlab=expression(tau),ylab=expression(paste("Wald(",tau,")",sep="")))
    tit <- paste(cr*100,c("% Critical"),sep="")
    legend("bottomright",c(expression(paste("Wald(",tau,")",sep="")),tit),lty=c(1,2),col=c(1,2))
    dev.off()
}
