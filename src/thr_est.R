...
if (graph==1){
    ...
    pdf(file="../output/graph-thr-est.pdf")
    plot(qs,lr,lty=1,col=1,xlim=xxlim,ylim=yylim,type="l",ann=0)
    lines(qs,clr,lty=2,col=2)     
    xxlab <- paste(c("Threshold Variable: Unemployment Rate"),sep="")
    title(main="Confidence Interval Construction for Threshold",
          xlab=xxlab,ylab=expression(paste("Likelihood Ratio Sequence in ", tau, sep="")))
    tit <- paste(conf1*100,c("% Critical"),sep="")
    legend("bottomright",c(expression(paste("LRn(",tau,")",sep="")),tit),lty=c(1,2),col=c(1,2))
    dev.off()
}
cat("95% horizontal line value: ", clr, '\n')
qhat
}
