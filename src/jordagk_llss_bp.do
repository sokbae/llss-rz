...
*** May 31, 2016
*** This program estimates IRFs and both 2-step and 1-step multipliers using ../output/ the Jorda method
...
capture log close;
log using ../output/jordagkirfs_results-bp.log, replace;
...
insheet using ../data/rz_dat_updated.csv;
...
outsheet h multlin2 multexp2 multrec2 using ../output/junk2step-bp.csv if h<=20, comma replace;

outsheet h byexp byrec bgexp bgrec seyexp seyrec segexp segrec using ../output/junk2stepirfs-bp.csv if h<=20, comma replace;
...
tw (rarea up95bglin lo95bglin h, bcolor(gs12) clw(medthin medthin)) 
  ...
  saving(../output/junkg-bp.gph,replace);
...
tw (rarea up95bylin lo95bylin h, bcolor(gs12) clw(medthin medthin)) 
  ...
  saving(../output/junky-bp.gph,replace);
 ...
 tw (rarea up95bgrec lo95bgrec h, bcolor(gs12) clw(medthin medthin))
  ...
  saving(../output/junkgnl-bp.gph,replace);
...
tw (rarea up95byrec lo95byrec h, bcolor(gs12) clw(medthin medthin))
  ...
  c(l l l l l) clp(- l - l) clc(red red red blue ) ms(i o i i i i) mc(red red red blue)) if h<=20,
  saving(../output/junkynl-bp.gph,replace);
...
outsheet h Fkplin Fkpexp Fkprec using ../output/junk-bp.csv if h<=20, comma replace ;
outsheet h Fdifflin Fdiffexp Fdiffrec using ../output/junkfdiff-bp.csv if h<=20, comma replace ;
...
outsheet h multlin1 multexp1 multrec1 using ../output/junk1step-bp.csv if h<=20, comma replace ;
...
outsheet h multlin1 seylin multexp1 seyexp multrec1 seyrec ptestdiff using ../output/junkmultse-bp.csv if h<=20, comma replace ;
